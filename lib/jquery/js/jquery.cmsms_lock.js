/**
 * @fileOverview code for cmsms_lockManager plugin.
 * @version 0.3
 * @author Robert Campbell
 */

/**
 * @name cmsms.lockManager
 * @namespace cmsms
 *
 * @example
 * $.cmsms_lock();
...
(function($) {
  $.widget('cmsms.lockManager', {
    options: {
      ...
      error_handler: null,
      change_noticed: false,
      lock_timeout: 60, // accepts >= 5
      lock_refresh: 60 // accepts 30..3600
//    secure_param, secure_value, admin_url, type set in _create() oid: and iud: set later
    },

    _settings: {
      locked: false,
      lock_id: -1, //ignored if <= 0
      lock_expires: -1, // set but not individually used here
      touch_skipped: false
      //other props set at runtime: lang[], ajax_url, oid, uid
    },
    ...
    _create: function() {
      ...
      if (typeof cms_data.secure_param_value !== 'undefined') { this.options.secure_value = cms_data.secure_param_value; }
      if (!this.options.secure_value) { throw 'The secure_value option (string) must be set in the cmsms_lock plugin'; }
      ...
      if (!this.options.type) { throw 'The type option (string) must be set in the cmsms_lock plugin'; }
      ...
      opts[this.options.secure_param] = this.options.secure_value;
      $.post(ajax_url, opts, function(data, textStatus) {
        ...
        delete data.status;
        self._settings = $.extend(self._settings, data); //TODO recursive merge
    ...
    _setup_touch: function() {
      ...
      var self = this;
      this._settings.touch_timer = setTimeout(function() {
        self._touch();
      }, interval * 1000);
    },
    ...
    _setup_handlers: function() {
      var self = this;
      this._settings.touch_skipped = false;
    ...
    _touch: function() {
      if (this.options.change_noticed && this._settings.locked && this._settings.lock_id > 0) {
        ...
        this._settings.touch_skipped = true;
      }
      this._setup_touch();
    },
