...
(function(global, $) {
    ...
    // polyfill for browsers without ES6 (before 2015 or so) remove ASAP
    // several uses across the core e.g. tinymce, filepicker
    ...
    /**
     * @description handles clicking on a cms_helpicon image
    ...
    cms_helper.cms_helpDialog = function() {
        ...
        $('.cms_help img.cms_helpicon').on('click', function() {
            ...
            if (key.length && $('#cmshelp_' + key).length === 0) {
                // get the text via ajax, put it in the div
                var e = $('<div class="cms_helptext" title="' + cms_lang('title_help') + ': ' + title + '" id="cmshelp_' + key + '" style="display: none;"></div>');
            ...
            var opts = Object.keys(data)
                .filter(function(key) {
                    return key.indexOf("dialog") === 0;
                })
                .reduce(function(obj, key) {
                    return Object.assign(obj, {
                      [key.substring(6)]: data[key]
                    });
                },
...
width: '40%'
    ...
    cms_helper.cms_initTabs = function() {
        ...
        if (tabs.filter('.active').trigger('mousedown').length === 0) { // WHAT?
            _cms_activateTab(0);
        }
...
/**
 * @description retrieve from cms_data a translated string for UI use
 * @function
 * @param (String) key The wanted language key
 * @return string or nothing
 */
function cms_lang(key) {
    'use strict';
    var lkey = 'lang_' + key;
    if (typeof(cms_data[lkey]) !== 'undefined') return cms_data[lkey];
    alert("cms data lang key '" + key + "' not set");
}
...
function cms_busy(flag) {
    ...
    if ($_div.length === 0) {
        // gotta add one.
        $('<div></div>',{id:'cms_busy','class':'busy'}).appendTo('body').hide();
    }
