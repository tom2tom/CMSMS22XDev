...
(function($) {
	$.widget('cmsms.filepicker', {
		...
		_create: function() {
			...
			this._settings.button = $('<button></button>',{'class':'cmsfp cmsfp-choose',text:this._settings.btn_label});
			...
			if (typeof this.options.required === 'undefined' || !this.options.required) {
				...
				var el = this._settings.clear = $('<button></button>').text(lbl).addClass('cmsfp cmsfp_clear');
			...
			if (!this._settings.url) {
				if (typeof cms_data.filepicker_url === 'undefined' || !cms_data.filepicker_url) throw "No filepicker_url in the cms_data class";
