...
(function($) {
  ...
  $.widget('cmsms.hierselector', {
    ...
    options: {
      current: 0, // numeric id of the current content item we are working on
      value: 0, // numeric id of the selected content item, or -1
      allowcurrent: false, // whether the user is allowed to select the current page,
      // and if current is greater than -1 then also allowed to select a child of that page
      use_perms: false, // whether to use pages' authorship permissions to control what is selectable
      use_simple: false, // whether to use a simple dropdown... implied if use_perms is true
      allow_all: false, // whether to show all content items i.e. include inactive and non-navigable ones 
    ...
    _build_smart_select: function(name, data, selected_id, parent_selectable, hilite) {
      ...
      sel.on('change', function() {
        ...
        self.data.hidden_e.val(v).trigger('change');
      ...
      if (parent_selectable) {
        ...
        self.data.hidden_e.val(selected_id).trigger('change');
      }
      for (var i = 0; i < data.length; i++) {
