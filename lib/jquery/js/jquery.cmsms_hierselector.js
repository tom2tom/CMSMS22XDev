...
(function($) {
  ...
  $.widget('cmsms.hierselector', {
    ...
    options: {
      ...
      use_perms: false, // whether to use pages' edit permission to control their display/selectability
...
/*  _setup_dropdowns: function() {
    ...
    _build_simple_select: function(name, data, cur_val, orig_page) {
      ...
      for (var idx = 0; idx < data.length; idx++) {
        ...
        if (this.options.use_perms && !data[idx].can_edit) opt.attr('disabled', 'disabled').addClass('nochildren'); // TODO why are permissions and children relevant here ?
...
//      if (!data[idx].has_children && this.options.use_perms && !data[idx].can_edit) opt.attr('disabled', 'disabled').addClass('nochildren');
          ...
          if (selected_id) { // > 0 ?
            for (var x2 = 0; x2 < data[idx].length; x2++) { // does nothing for selected_id == -1 unless the following can match
              if (data[idx][x2].content_id == selected_id) { // -1 never matched unless cloned page is being processed?
                if (has_optnone) {
                  if (!(self.options.for_child || data[idx][x2].has_usable_link)) { //has_usable_link false for types ErrorPage, Separator etc (no child pages) TODO allow_all setting should prevail?
