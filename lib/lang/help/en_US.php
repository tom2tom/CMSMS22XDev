...
<p>Some third party modules provide more content types to serve different purposes. Such as displaying catalogues of products, or restricting the content to authorized users.</p>
...
<ul>
 ...
 <li>Whether the content item is being edited
  <p>When other users <em>(or even yourself)</em> are editing a content item, some columns will be hidden in the row for each content type to prevent modifying, deleting or copying the content page.</p>
 </li>
</ul>
<h5>Column List</h5>
...
<ul>
 ...
 <li>Reorder Pages
  ...
  <p>e.g. a user in a group who has edit permission for the content item with hierarchy 1.3 and all of its direct siblings <em>(1.1, 1.2, 1.3, 1.4, etc)</em> will be able to re-arrange those items in the navigation. Users without this permission will not see the move up/down icons in the content list.</p>
 </li>
...
<p>It is possible for an Admin user account to not be a member of any groups, and for that Admin user account still have the ability <em>(as an owner or additional editor)</em> to edit some content items.</p>
 ...
 <li>Bulk Actions
  ...
  <ul>
   ...
   <li>Set Cachable
    ...
    <p>Also in &quot;Site Admin >> Global Settings&quot; Smarty caching affects cachable pages. This is an advanced tool that will cache the generated html code of a for repeated use, and can dramatically reduce server load and improve performance. However, it is an advanced topic and may negatively the dynamic nature of some content items.</p>
    <p>Only users with the &quot;Manage All Content&quot; permission can use this option.</p>
   </li>
   <li>Set Not Cachable
...
<ul>
 ...
 <li>Site settings <em>(e.g: the &quot;Basic Properties&quot; field in the &quot;Global Settings&quot; window, and others)</em>.
...
<ul>
 ...
 <li>Active <em>(i.e: not disabled)</em>
 ...
 <li>Disable WYSIWYG
  <p>This property will disable the WYSIWYG editor for all content blocks on this content item. This overrides all settings on the content blocks, and any user setting. This is useful for content items that contain pure logic in the content blocks, or strictly call other modules. This prevents the logic or output from the modules from being affected by the styling injected by the WYSIWYG.</p>
  <p>Users with restricted permissions may not have the ability to adjust or specify this property.</p>
 </li>
 <li>Page Metadata
...
<p>The Design Manager admin interface allows filtering templates by originator, or type, to easily find templates to edit or manage.</p>
...
$lang['help_jobs'] = <<<'EOT'
<p>Listed jobs (if any) might have been recorded before (not during) the request which displayed the list. That can happen if an error prevented completion of a fresh snapshot, or if the latest snapshot is relatively recent. How long ago they were recorded depends on the configured processing interval and when previous requests actually occurred.<br>
It is not uncommon for jobs to frequently appear in and disappear from the list.<br>
<br>
'Logged' values displayed in the list represent when the job was recorded. Such values are displayed only when they differ from the corresponding start value.<br>
'Frequency' values displayed in the list are nominal.  Actual frequencies will typically be lower, as they also depend on how often requests are processed at the site.<br>
'Until' values displayed in the list are for the site's configured timezone unless otherwise indicated.<br>
<br>
Any job which should have started, but did not, should be reported to the appropriate developer for diagnosis.<br>
<br>
Any job might fail to complete. Any job which has failed a number of times will be discarded from the waiting-jobs list, but after discard, the originating code can re-create the job. If the job then continues to fail, this should be reported in detail to the appropriate developer for diagnosis.</p>
EOT;

$lang['help_jobsconfigure'] = <<<'EOT'
<p>Authorized users can change several job-processing settings, via the Site Amin Global Settings page. Some of those settings can be overridden by the configuration properties described below.<br>
<br>
Site developers can add Job and/or Task classes into this site's 'assets/jobs' folder to perform additional async operations. Module developers can deploy Job and/or Task classes which are reported by the module's get_tasks() method.<br>
<br>
A description can be assigned to each Job. That would be used as a tooltip in the Jobs list.<br>
A priority (1-3, default 2) can be assigned to each Job. That might be useful in congested situations.<br>
<br>
Site developers can adjust the processing interval by adding into the config.php file for the site a 'cmsjobmanager_asyncfreq' property having an integer value from 3 to 60 (minutes between jobs-checks) e.g.
<pre><code>$config['cmsjobmanager_asyncfreq'] = 5;</code></pre>.
<br>
<strong>Note:</strong> It is not possible to disable asynchronous processing, because some functioning of the CMSMS core relies on such processing.<br>
<br>
Site developers can override PHP's 'max_execution_time' setting to set a different timeout for processing each batch of jobs, by adding into the config.php file for the site a 'cmsjobmanager_timelimit' property having an integer value from 10 to 600 (seconds) e.g.
<pre><code>$config['cmsjobmanager_timelimit'] = 20;</code></pre>.
<br><br>
On low-traffic sites, it might be desirable for background processing to occur more often than requests. If so, it is recommended that a developer sets up a cron job to regularly request an URL on the site e.g.
<pre>*/3 * * * * wget -O /dev/null https://www.mysite.com/index.php?page=dojobs&showtemplate=false</pre>
<br>
The referenced page would be a simple content page with no content of its own and that is not shown in the site-navigation menu.</p>
EOT;
...
$lang['settings_contentimage_path'] = 'This is an optional sub-path, relative to the site\'s configured \'uploads_path\' <strong>NOT \'image_uploads_path\'</strong>. Without surrounding path-separator characters. If not empty, image files for {content_image} tags will need to be stored in and will be retrieved from the resultant joined folder.<br>If that folder does not exist now, it will be created.';
$lang['settings_cssnameisblockname'] = 'If enabled, the content block name <em>(id)</em> will be used as a default value for the cssname parameter for each content block.<br>This is useful for WYSIWYG editors.  The stylesheet (block name) can be loaded by the WYSIWYG editor and provide an appearance that is closer to that of the front web page.<br><br><strong>Note:</strong> WYSIWYG editors might fail to read information from the supplied stylesheets (if they exist) depending upon their settings and capabilities.';
$lang['settings_imagefield_path'] = 'This is an optional sub-path, relative to the site\'s configured \'image_uploads_path\'. Without surrounding path-separator characters. If not empty, content-page image files will need to be stored in and will be retrieved from the resultant joined folder.<br>If that folder does not exist now, it will be created.';
...
$lang['settings_nosefurl'] = 'To configure <strong>S</strong>earch <strong>E</strong>ngine <strong>F</strong>riendly <em>(pretty)</em> URLs you need to edit a few lines in the site\'s config.php file and possibly to edit a .htaccess file or the web server\'s configuration.  You can read more about configuring pretty URLs <a href="https://docs.cmsmadesimple.org/configuration/pretty-url" class="external" target="blank"><u>here</u></a> &raquo;';
$lang['settings_thumbfield_path'] = 'This is an optional sub-path, relative to the site\'s configured \'image_uploads_path\'. Without surrounding path-separator characters. If not empty, content-page thumbnail image files will need to be stored in and will be retrieved from the resultant joined folder.<br>If that folder does not exist now, it will be created.';
// these 'settings_*' are used in the siteprefs template and could be 'siteprefs_*'
...
//$lang['settings_disablesafemodewarn'] = 'This option will disable a warning notice if CMSMS detects that <a href="https://php.net/manual/en/features.safe-mode.php" class="external" target="_blank">PHP Safe Mode</a> has been detected.<br><br><strong>Note:</strong> Safe mode has been deprecated as of PHP 5.3.0 and removed for PHP 5.4.0.  CMSMS Does not support operation under safe mode, and the CMSMS support team will not provide technical assistance for installs where safe mode is active';
...
$lang['settings_mailprefs_smtp_smtpautotls'] = 'When using the SMTP mailer, this option indicates whether to enable TLS encryption automatically, if a server supports it. This setting is valid even if the <em>Encryption Method</em> is not set to <em>TLS</em>.It also requires that the server\'s certificates are valid.<br>Some rare hosting providers impose very strict (to the point of becoming almost useless) constraints on sending an email. They block outbound SMTP to ports 25, 465 and 587 to all servers except their own. If you find your script works on your local machine, but not when you upload it to a server, this may be what is happening. The solution is poorly documented in most cases: sending through their servers, while disabling all security features, such as username, password and auto TLS encryption, which is <strong>on</strong> by default.';
...
$lang['settings_searchmodule'] = 'Select the module that will be used to index words for searching, and will provide the site search capabilities';
...
$lang['settings_smartycache_adminlife'] = 'Specify the number of minutes that compiled admin-console-page templates will remain in Smarty\'s cache (if enabled). Setting this value to 0 effectively disables admin caching. Smarty\'s default is 60.';
$lang['settings_smartycache_frontlife'] = 'Specify the number of minutes that compiled frontend-page templates will remain in Smarty\'s cache (if enabled). Setting this value to 0 effectively disables frontend caching. Smarty\'s default is 60.';
...
$lang['siteprefs_dateformat'] = 'Specify the date format string in <a href="https://www.php.net/manual/en/function.strftime.php" class="external" rel="noopener noreferrer" target="_blank"><u>PHP strftime</u></a> format that will be used <em>(by default)</em> to display dates and times in your website.<br>Admin users can adjust these settings in the user preferences panel.<br><br><strong>Note:</strong> some modules may choose to display times and dates differently.';
$lang['siteprefs_frontendlang'] = 'The default language that this website displays on frontend pages.  This can be changed on a per-page basis using a Smarty tag i.e. <code>{cms_set_language}</code>';
...
$lang['siteprefs_jobmaxerrs'] = 'Enter an integer in the range 0 to 20, to be the allowed number of errors encountered when processing a job before that job will be automatically removed from the the list';
$lang['siteprefs_jobsinterval'] = 'Enter an integer in the range 3 to 60, to be the minimum number of minutes between background checks for and processing of pending jobs.<br>This value can be overridden by a config-file property \'cmsjobmanager_asyncfreq\'';
$lang['siteprefs_jobstimeout'] = 'Enter an integer in the range 20 to 600, to be the maximum number of seconds allowed for processing each batch of jobs in the background.<br>This value can be overridden by a config-file property \'cmsjobmanager_timelimit\'';
...
$lang['siteprefs_protected_path'] = 'Enter an absolute or site-root-relative folder-path. With commas separating path segments and without surrounding commas or any invalid whitespace. May contain \'..\' segment(s) and/or $config[\'some_property\'] segment(s), but no other variables or constants. If crafted appropriately, the resolved result might be outside of the site folders-tree.';
$lang['siteprefs_sitename'] = 'Provide a human-readable name for the website e.g. the business, club, or organization name';
