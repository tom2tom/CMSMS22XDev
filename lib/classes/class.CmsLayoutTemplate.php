...
class CmsLayoutTemplate
{
	 ...
	 * @ignore
	 * @internal
	 * should be private but _load_from_data() needs public
	 */
	public $_data = [
	'id' => 0,
	'name' => '',
	...
	/**
	 * Get the type id of this template
	 *
	 * @return int maybe 0
	 */
	public function get_type_id()
	{
		return (int)$this->_data['type_id'];
	}
	...
	/**
	 * Get the flag indicating whether this template is the default template for its type
	 *
	 * @return bool
	 * @see CmsLayoutTemplateType
	 */
	...
	{
		return (bool)$this->_data['type_dflt'];
	}
	 ...
	 *
	 * @return mixed CmsLayoutTemplateCategory | null
	 */
	...
	/**
	 * Get the design id's (if any) that this template is associated with
	 *
	 * @return array integer ids, or empty
	 */
	...
	/**
	 * Set the design id's (if any) that this template is associated with
	 ...
	 *
	 * @return int, -1 if no owner is recorded
	 */
	public function get_owner_id()
	{
		return (int)$this->_data['owner_id'];
	}
	...
	public function set_owner($a)
	{
		...
		elseif( is_string($a) && strlen($a) ) {
			...
			if( is_object($a) && is_a($a,'User') ) $n = (int)$a->id;
		}
		elseif( is_object($a) && is_a($a,'User') ) {
			$n = (int)$a->id;
		}
	...
	/**
	 * Get the timestamp representing when this template was first recorded in the database
	 * This property cannot be externally modified
	 *
	 * @return int maybe 0
	 */
	...
	/**
	 * Get the timestamp representing wnen this template was last saved
	 * This property cannot be externally modified
	 *
	 * @return int maybe 0
	 */
	...
	/**
	 * Get whether the specified user has edit authority for this template object
	 *
	...
	public function can_edit($a)
	{
		...
		return ( is_array($this->_addt_editors) && count($this->_addt_editors) && in_array($res,$this->_addt_editors) ) ;
	}
	...
	/**
	 * Set whether this template is listable in public template lists.
	 *
	 * @param mixed $flag bool|int|string supported by cms_to_bool() Default true
	 * @since 2.1
	 */
	...
	protected function validate()
	{
		...
		if( $tid > 0 ) {
			...
			$dbr = $db->GetOne($query,array($this->get_name(),$tid));
		}
		else {
			// double check the name.
...
WHERE id = ?';
		$pd = ($this->get_type_dflt()) ? 1 : 0;
		$pl = ($this->get_listable()) ? 1 : 0;
		$db = CmsApp::get_instance()->GetDb();
		$db->Execute($query,
							array($this->get_name(),$this->get_content(),$this->get_description(),
								  $this->get_type_id(),$pd,$this->get_category_id(),
								  $this->get_owner_id(),$pl,time(),$tid));
		...
		if( is_array($t) && count($t) ) {
			...
			foreach( $t as $one ) {
				$dbr = $db->Execute($query,array($tid,(int)$one));
			}
		}
		...
		audit($tid,'Template',"Updated: {$this->get_name()}");
		$this->_dirty = FALSE;
	}
...
VALUES (?,?,?,?,?,?,?,?,?,?)';
		...
		$dbr = $db->Execute($query,
							array($this->get_name(),$this->get_content(),$this->get_description(),
								  $this->get_type_id(),$pd,$this->get_category_id(),
								  $this->get_owner_id(),$pl,$now,$now));
		if( !$dbr ) throw new CmsSQLErrorException($db->sql.' -- '.$db->ErrorMsg());
		$this->_data['id'] = $db->Insert_ID();
		$tid = (int)$this->_data['id'];
		...
		$this->_dirty = FALSE;
		CmsTemplateCache::clear_cache();
		audit($tid,'Template',"Created: {$this->get_name()}");
	}
	...
	/**
	 * Test if this template is locked by an expired lock.
	...
	private static function _load_from_data($row,$design_list = [])
	{
		...
		self::$_name_cache[$ob->get_name()] = $tid;// aka [$row['name']]
		return $ob;
	}
	 ...
	 * @return array of CmsLayoutTemplates
	 */
	...
	public static function get_editable_templates($a)
	{
		...
		elseif( is_array($tmp1) ) {
			$tmp = $tmp1;
		}
		elseif( is_array($tmp2) ) {
			$tmp = $tmp2;
		}
	 ...
	 */
	public static function user_can_edit($tpl,$userid = 0)
	{
	...
	public static function create_by_type($t)
	{
		...
		elseif( is_object($t) && is_a($t,'CmsLayoutTemplateType') ) {
			$t2 = $t;
		}
	...
	public static function load_dflt_by_type($t)
	{
		...
		if( self::$_obj_cache && is_array(self::$_obj_cache) ) {
			foreach( self::$_obj_cache as $tpl ) {
				if( $tpl->get_type_id() == $tid2 && $tpl->get_type_dflt() ) {
					return $tpl;
				}
			}
		}
		...
		$dbr = $db->GetRow($query,array($tid2));
		if( !$dbr ) throw new CmsDataNotFoundException('Could not find default CmsLayoutTemplate row for type '.$t2->get_name());

		return self::_load_from_data($dbr);
	}
	...
	public static function load_all_by_type(CmsLayoutTemplateType $type)
	{
		...
		$dbr = $db->GetArray($query,array($type->get_id()));
		if( !$dbr ) return [];
	 ...
	 *
	 * @return CmsLayoutTemplateType | null
	 * @since 2.2
	 */
	public function get_type()
	{
		...
		if( $tid > 0 ) {
			$obj = \CmsLayoutTemplateType::load($tid);
		}
		else {
			$obj = null;
		}
	...
	/**
	 * Get the name of the file used or potentially used to record this template's contents.
	 *
	 * @since 2.2
	 * @return string
	 */
	...
	/**
	 * Is this template recorded in a filesystem file?
	 *
	 * @since 2.2
	 * @return bool
	 */
