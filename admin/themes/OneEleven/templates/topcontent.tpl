	...
	<div class="dashboard-box{if $node@index % 3 == 2} last{/if}">
		...
		{/if}
			...
			{foreach $node.children as $one}
				<li><a href="{$one.url}"{if isset($one.target)} target="{$one.target}"{/if}{if (isset($is_sitedown) && strncmp($one.url,'logout',6) == 0)} onclick="return confirm('{lang('maintenance_warning')|escape:'javascript'}');"{/if}>{$one.title}</a></li>
			{/foreach}
			</ul>
			{/if}
		</nav>
