...
/**
 * A class defining a database connection, and mechanisms for working
 * with the database.
 *
 * This class is largely compatible with adodb_lite with the
 * pear, extended, transaction plugins and with one notable difference:
 * <ul>
 *  <li>GenID will not automatically create a sequence table</li>
 * </ul>
 *
 * @abstract
 * @package CMS
...
abstract class Connection
{
     ...
     * Used only in error handling. Must restrict confidential-properties display.
     *
     * @internal
     */
    ...
    public function GetAssoc( $sql, $inputarr = [], $force_array = false, $first2cols = false )
    {
        $result = $this->SelectLimit($sql, -1, -1, $inputarr );
        if( $result ) {
            return $result->GetAssoc($force_array,$first2cols);
        }
        return [];
    }
     ...
     * @return array A single flat array of results, one entry per row matched, or empty.
     */
    public function GetCol($sql, $inputarr = [], $trim = false)
    {
        $result = $this->SelectLimit($sql, -1, -1, $inputarr);
        if ($result) {
            $key = key($result->Fields());
            $data = array_column($result->GetArray(), $key);
            if ($trim) {
    ...
    {
        $nrows = (stripos($sql, 'LIMIT') !== FALSE) ? -1 : 1;
        $result = $this->SelectLimit( $sql, $nrows, -1, $inputarr );
...
class DatabaseException extends \LogicException
{
     ...
     *
     * @return \CMSMS\Database\ConnectionSpec with most properties obfuscated
     */
