...
{if $pmod}
  ...
  <div class="pageoptions"><a href="{cms_action_url action=edit_guide gid=0}">{admin_icon icon='newobject.gif'} {$addname}</a></div>
{else}
<br>
{/if}
{if !empty($guides)}
...
{if $pmod}      <th class="pageicon"></th>{*edit icon*}
    ...
    <tbody class="sortable-list">{if $pmod}{assign 'aname' 'edit_guide'}{assign 'tkey' 'edit'}{else}{assign 'aname' 'view_guide'}{assign 'tkey' 'display'}{/if}
...
{/if}
      ...
      <td class="pagepos">{if $pmod}<a class="active_guide" href="{cms_action_url action=toggle_active_guide gid=$one->id}" title="{$mod->Lang('toggle_active')}">{/if}{if $one->active}{admin_icon icon='true.gif'}{else}{admin_icon icon='false.gif'}{/if}{if $pmod}</a>{/if}
      </td>
      <td class="pagepos">{if $pmod}<a class="search_guide" href="{cms_action_url action=toggle_search_guide gid=$one->id}" title="{$mod->Lang('toggle_search')}">{/if}{if $one->search}{admin_icon icon='true.gif'}{else}{admin_icon icon='false.gif'}{/if}{if $pmod}</a>{/if}
      </td>
      <td class="pagepos">{if $pmod}<a class="admin_guide" href="{cms_action_url action=toggle_admin_guide gid=$one->id}" title="{$mod->Lang('toggle_admin')}">{/if}{if $one->admin}{admin_icon icon='true.gif'}{else}{admin_icon icon='false.gif'}{/if}{if $pmod}</a>{/if}
      </td>
      <td>{if $one->active}<a href="{cms_action_url action='view_guide' gid=$one->id}" title="{$mod->Lang('display')}">{admin_icon icon='view.gif'}</a>{/if}</td>
{if $pmod}      <td><a href="{$action_url}" title="{$mod->Lang('edit')}">{admin_icon icon='edit.gif'}</a></td>
      <td><a class="del_guide" href="{cms_action_url action=delete_guide gid=$one->id}" title="{$mod->Lang('delete')}" data-title="{$one->name}">{admin_icon icon='delete.gif'}</a></td>
{/if}
    </tr>
{/foreach}
    </tbody>
