...
$(function() {
  ...
  $('#template_area').autoRefresh({
    ...
    done_handler: function() {
      $('#tpl_bulk_sel,#tpl_bulk_submit').prop('disabled', true);
...
<div id="filterdialog" style="display:none" title="{$mod->Lang('tpl_filter')|escape:'javascript'}">
  {form_start action='defaultadmin' id='filterdialog_form' __activetab='templates'}
    ...
    <div style="display:table-row">
      <label for="filter_tpl" class="endalign" style="display:table-cell;padding-{$ndside}:.5em">{$mod->Lang('prompt_options')}:</label>
      <select id="filter_tpl" style="display:table-cell" name="{$actionid}filter_tpl" title="{$mod->Lang('title_filter')}">
        {html_options options=$filter_tpl_options selected=$tpl_filter.tpl}
      </select>
    </div>
    <div style="display:table-row">
      <label for="filter_sortby" class="endalign" style="display:table-cell;padding-{$ndside}:.5em">{$mod->Lang('prompt_sortby')}:</label>
      <select id="filter_sortby" style="display:table-cell" name="{$actionid}filter_sortby" title="{$mod->Lang('title_sortby')}">
    ...
    <div style="display:table-row">
      <label for="filter_sortorder" class="endalign" style="display:table-cell;padding-{$ndside}:.5em">{$mod->Lang('prompt_sortorder')}:</label>
      <select id="filter_sortorder" style="display:table-cell" name="{$actionid}filter_sortorder" title="{$mod->Lang('title_sortorder')}">
    ...
    <div style="display:table-row">
      <label for="filter_limit" class="endalign" style="display:table-cell;padding-{$ndside}:.5em">{$mod->Lang('prompt_limit')}:</label>
      <select id="filter_limit"  style="display:table-cell" name="{$actionid}filter_limit_tpl" title="{$mod->Lang('title_filterlimit')}">
